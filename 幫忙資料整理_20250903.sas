...
/* ===== A) 建立基表（去掉公司/年份，保留舊 DMU 名稱以便排序）===== */
data work.ESG_DMU_reg_base;
  set work.ESG_DMU_whole(drop=companyID companyName year);
  rename DMU=DMU_old;
run;

/* ===== B) 依 certificate 分三組（先切再編號，DMU 放在 certificate 前面）===== */
%macro split_and_renumber(tag=, cond=);
  data _cert&tag._base;
    set work.ESG_DMU_reg_base;
    where &cond;
  run;

  proc sort data=_cert&tag._base; by DMU_old; run;

  data ESG_DMU_reg_&tag._base;
    retain DMU certificate;  /* DMU 會排到 certificate 前 */
    set _cert&tag._base;
    retain __idx 0;
    __idx + 1;
    DMU = cats('DMU', put(__idx, z5.));
    drop __idx DMU_old;
  run;
%mend;

%split_and_renumber(tag=Y,  cond= upcase(strip(certificate))='Y');
%split_and_renumber(tag=N,  cond= upcase(strip(certificate))='N');
%split_and_renumber(tag=YN, cond= upcase(strip(certificate)) in ('Y','N'));

/* ===== C) 針對每組輸出三種 ESG 欄位保留方式 ===== */
%macro emit_three(cert=);
  /* 只留 ESG_score（移除 ESG_E / ESG_S / ESG_G） */
  data work.ESG_DMU_reg_&cert._score;
    set ESG_DMU_reg_&cert._base;
...
/* ===== D) 指定「最終欄位順序」並套用到 9 個表 ===== */
%macro _reorder_to(ds=);
  %let _target_order=
    DMU certificate
  ...
  %_filter_keep(ds=&ds, list=&_target_order, out=_ord_);
  %if %superq(_ord_) ne %then %do;
    data &ds;
      retain &_ord_;
      set &ds(keep=&_ord_);
    run;
...
%macro reorder9;
  ...
  %do i=1 %to 3;
    ...
    %do j=1 %to 3;
      ...
      %_reorder_to(ds=work.ESG_DMU_reg_&cert._&type);
    %end;
  %end;
%mend;
