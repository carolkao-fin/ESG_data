...
/* ===== 5.5) 產生 DMU（按 year、companyID 排序；DMU 放 companyName 後）===== */
proc sort data=work.ESG_merged out=work.ESG_merged_s;
  by year companyID;
run;
...
data work.ESG_merged_DMU;
  ...
  length DMU $8;
  DMU = cats('DMU', put(_N_, z5.));  /* DMU00001, DMU00002, ... 不按年份重置 */
run;
