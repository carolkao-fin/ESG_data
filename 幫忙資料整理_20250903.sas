...
/* ============================================
   ...
   - 刪 ROU_asset/income/cost/expenditure/PPE/RD/NGW/OtherIntan 任一「存在且缺失」
...
%macro build_full_then_short();
  ...
  %do %while(%length(&one));
    ...
    data work._filtered0;
      ...
      /* ===== 缺失與負值檢查（存在才檢）===== */
      length __miss_any 3; __miss_any = 0;

      %if &_has_rou %then %do;
        length __rou 8;
        if vtype(ROU_asset)='C' then __rou = inputn(strip(ROU_asset),'best32.');
        else __rou = ROU_asset;
        if missing(__rou) then __miss_any=1;
      %end;

      %if &_has_inc %then %do;
        length __inc 8;
        if vtype(income)='C' then __inc = inputn(strip(income),'best32.');
        else __inc = income;
        if missing(__inc) then __miss_any=1;
      %end;

      %if &_has_cost %then %do;
        length __cost 8;
        if vtype(cost)='C' then __cost = inputn(strip(cost),'best32.');
        else __cost = cost;
        if missing(__cost) then __miss_any=1;
      %end;

      %if &_has_exp %then %do;
        length __exp 8;
        if vtype(expenditure)='C' then __exp = inputn(strip(expenditure),'best32.');
        else __exp = expenditure;
        if missing(__exp) then __miss_any=1;
      %end;

      %if &_has_ppe %then %do;
        length __ppe 8;
        if vtype(PPE)='C' then __ppe = inputn(strip(PPE),'best32.');
        else __ppe = PPE;
        if missing(__ppe) then __miss_any=1;
      %end;

      %if &_has_rd %then %do;
        length __rd 8;
        if vtype(RD)='C' then __rd = inputn(strip(RD),'best32.');
        else __rd = RD;
        if missing(__rd) then __miss_any=1;
      %end;

      %if &_has_ngw %then %do;
        length __ngw 8;
        if vtype(NGW)='C' then __ngw = inputn(strip(NGW),'best32.');
        else __ngw = NGW;
        if missing(__ngw) then __miss_any=1;
      %end;
      ...
      %if &_has_oi %then %do;
        ...
        /* ① OtherIntan 缺失 → 視為缺失條件命中 */
        if missing(__oi) then __miss_any=1;
        /* ② OtherIntan < 0 → 直接刪除 */
        if not missing(__oi) and __oi<0 then delete;
      %end;

      /* 若上面任一存在欄位為缺失 → 刪除 */
      if __miss_any=1 then delete;

      drop _ys __miss_any __rou __inc __cost __exp __ppe __rd __ngw __oi;
    run;
    ...
    %put NOTE: FILE &one : Dropped by rules      = %eval(&_n_before - &_n_after) ;

    /* 5) 重新編 DMU（先 drop 舊 DMU 再重建） */
    ...
    %put NOTE: Wrote FULL to &out_full ;

    /* 7) 由 full_del 直接產生 short_del（只刪三欄 companyID companyName year） */
    ...
    %put NOTE: Wrote SHORT to &out_short ;
